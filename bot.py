...
def db_init():
    ...
    set_default("backup_hours", "24")               # interval hours
    set_default("backup_target_id", str(ADMIN_CHAT_ID))  # where to send
...
def kb_day_menu(date_g: str, day_rows: List[sqlite3.Row]) -> InlineKeyboardMarkup:
    ...
    for title, ttype in groups:
        ...
        if not items:
            buttons.append([InlineKeyboardButton("خالی", callback_data="noop")])
            continue

        for r in items[:40]:
            cb = f"item:open:{r['id']}:{date_g}"
            buttons.append([
                InlineKeyboardButton(f"{r['category']}", callback_data=cb),
...
def day_text(user_id: int, date_g: str) -> str:
    ...
    scope, _owner = current_scope(user_id)
...
async def send_backup_file(bot, chat_id: int, caption: str = ""):
    ...
    try:
        await bot.send_document(chat_id=chat_id, document=InputFile(backup_name),
                                caption=caption or f"✅ بکاپ: {backup_name}")
    finally:
        try:
...
async def on_tx_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.data == "tx:date:greg":
        await q.edit_message_text(
            "تاریخ میلادی را وارد کن (YYYY-MM-DD):",
    ...
    if q.data == "tx:date:jal":
        await q.edit_message_text(
            "تاریخ شمسی را وارد کن (YYYY-MM-DD):\nمثال: 1404-10-11",
            reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("↩️ بازگشت", callback_data="m:tx")]])
        )
        return ST_JAL_DATE
...
async def on_cat_del_value(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    grp = context.user_data["cat_grp"]
    name = (update.message.text or "").strip()
    okk, msg = del_category(update.effective_user.id, grp, name)
    await update.message.reply_text(("✅ " if okk else "⚠️ ") + msg, reply_markup=kb_cat_actions(grp))
    return ConversationHandler.END
...
async def on_db_backup(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if q.from_user.id != ADMIN_CHAT_ID:
        return
    await send_backup_file(context.bot, q.from_user.id, caption="✅ گرفتن بکاپ دستی")
...
async def on_db_import_start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text(
        "📥 وارد کردن بکاپ\n\n"
        "فایل بکاپ دیتابیس (.db) را ارسال کن.\n"
        "⚠️ با وارد کردن بکاپ، دیتابیس فعلی جایگزین می‌شود.",
...
async def on_db_import_file(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not doc:
        await update.message.reply_text("لطفاً فایل .db ارسال کن.")
        return ST_DB_IMPORT_FILE
    ...
    if not is_sqlite_file(tmp_name):
        try:
            ...
            pass
        await update.message.reply_text("این فایل دیتابیس SQLite معتبر نیست.")
        return ConversationHandler.END
    ...
    await update.message.reply_text(
        f"✅ بکاپ وارد شد.\n"
        f"🔁 دیتابیس فعلی جایگزین شد.\n"
        f"🧾 نسخه پشتیبان قبلی: {bak_name}"
    )
...
async def on_db_auto_toggle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await q.edit_message_text("✅ تنظیم شد.", reply_markup=kb_db_admin())
...
async def on_backup_hours(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    txt = (update.message.text or "").strip()
    if not txt.isdigit():
        await update.message.reply_text("فقط عدد بفرست. مثال: 6")
        return ST_BACKUP_HOURS

    hours = int(txt)
    if hours <= 0 or hours > 720:
        await update.message.reply_text("عدد منطقی بفرست (1 تا 720).")
        return ST_BACKUP_HOURS

    cfg_set("backup_hours", str(hours))

    await update.message.reply_text(
        f"✅ هر {hours} ساعت تنظیم شد.\n\n"
        f"حالا آیدی مقصد بکاپ را بفرست (عدد).\n"
        f"اگر خالی می‌خوای همون ادمین باشد، عدد {ADMIN_CHAT_ID} را بفرست."
    )
...
async def on_backup_target(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    txt = (update.message.text or "").strip()
    if not txt.isdigit():
        await update.message.reply_text("فقط عدد بفرست (آیدی عددی).")
        return ST_BACKUP_TARGET
...
def build_app() -> Application:
    ...
    # -------------------------
    # ConversationHandler (اولویت بالا)  ✅✅✅
    # -------------------------
    conv = ConversationHandler(
        ...
        # ✅ مهم برای مکالمه‌هایی که با دکمه (CallbackQuery) شروع می‌شن
        per_message=True,
    )

    # conv رو اول اضافه می‌کنیم که هیچ handler دیگه‌ای state رو ندزده
    app.add_handler(conv, group=0)

    # -------------------------
    # سایر handler ها (اولویت پایین‌تر)
    # -------------------------
    app.add_handler(CommandHandler("start", cmd_start), group=1)

    app.add_handler(CallbackQueryHandler(on_menu, pattern=r"^m:(home|tx|rep|set)$"), group=1)
    app.add_handler(CallbackQueryHandler(on_reports_month, pattern=r"^rep:month:\d{4}:\d{1,2}$"), group=1)
    app.add_handler(CallbackQueryHandler(on_report_detail, pattern=r"^rep:detail:(work_in|work_out|personal_out):\d{4}:\d{1,2}$"), group=1)

    app.add_handler(CallbackQueryHandler(on_day_open, pattern=r"^day:open:\d{4}-\d{2}-\d{2}$"), group=1)
    app.add_handler(CallbackQueryHandler(on_item_open, pattern=r"^item:open:\d+:\d{4}-\d{2}-\d{2}$"), group=1)
    app.add_handler(CallbackQueryHandler(on_item_delete, pattern=r"^item:delete:\d+:\d{4}-\d{2}-\d{2}$"), group=1)

    app.add_handler(CallbackQueryHandler(on_set_cats, pattern=r"^set:cats$"), group=1)
    app.add_handler(CallbackQueryHandler(on_cat_group, pattern=r"^cat:grp:(work_in|work_out|personal_out)$"), group=1)
    app.add_handler(CallbackQueryHandler(on_cat_list, pattern=r"^cat:list:(work_in|work_out|personal_out)$"), group=1)

    app.add_handler(CallbackQueryHandler(on_access_menu, pattern=r"^set:access$"), group=1)
    app.add_handler(CallbackQueryHandler(on_access_set, pattern=r"^acc:set:(admin_only|allowed_users|public)$"), group=1)
    app.add_handler(CallbackQueryHandler(on_allowed_menu, pattern=r"^acc:allowed:menu$"), group=1)
    app.add_handler(CallbackQueryHandler(on_allowed_users, pattern=r"^acc:users$"), group=1)
    app.add_handler(CallbackQueryHandler(on_allowed_list, pattern=r"^acc:users:list$"), group=1)
    app.add_handler(CallbackQueryHandler(on_share_toggle, pattern=r"^acc:share:toggle$"), group=1)

    app.add_handler(CallbackQueryHandler(on_db_menu, pattern=r"^set:db$"), group=1)
    app.add_handler(CallbackQueryHandler(on_db_backup, pattern=r"^db:backup$"), group=1)
    app.add_handler(CallbackQueryHandler(on_db_auto_toggle, pattern=r"^db:auto:toggle$"), group=1)
    app.add_handler(CallbackQueryHandler(on_db_auto_config, pattern=r"^db:auto:config$"), group=1)

    app.add_handler(CallbackQueryHandler(on_noop, pattern=r"^noop$"), group=1)
