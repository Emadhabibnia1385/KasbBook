---
name: kasbbook
description: Engineering constitution and safe-work protocol for KasbBook — the Persian/Jalali small-business bookkeeping bot and API (Python 3.12, FastAPI, SQLAlchemy 2 async, PostgreSQL, Alembic, Telegram/Bale/Rubika adapters, double-entry ledger). Read this BEFORE making any change in this repo — feature, bug fix, migration, screen, endpoint, or "quick tweak". Use it whenever the task touches src/kasbbook/ (adapters, bot/conversation, bot/screens, api/routers, modules/*/service.py, shared/money or jalali), apps/bot/runner.py, migrations/, deploy/, scripts/install-update-backup-restore, tests/v2/, or anything about books, transactions, the ledger, budgets, debts, loans, recurring rules, payroll, treasury, member shares, identities and messenger linking, refresh tokens, or deploying to the server. Consult it even when the change looks small or nobody said "use the skill" — this codebase has specific invariants that are enforced by tests, and several failures here were silent rather than loud.
---
